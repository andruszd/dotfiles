...
# Set version in docs/conf.py
grep -E '^release = "(.+)"' "$ROOT/docs/conf.py" >/dev/null
sed -i.bak -E "s/^release = \"(.+)\"/release = \"$1\"/" "$ROOT/docs/conf.py" && rm "$ROOT/docs/conf.py.bak"

# Build collection package with final version numbers.
