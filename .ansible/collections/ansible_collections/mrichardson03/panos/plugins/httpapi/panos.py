...
class PanOSAPIError(ConnectionError):
    ...
    def __init__(self, code, message):
        if code not in _PANOS_API_ERROR_CODES:
            ...
            self._code = code
            msg = f"{_PANOS_API_ERROR_CODES[code]} ({code})"
...
class HttpApi(HttpApiBase):
    ...
    def version(self, refresh=False):
        ...
        display.vvvv(f"version = {self._device_info}")
    ...
    def poll_for_job(self, job_id, interval=5, timeout=600):
        ...
        display.vvvv(
            f"poll_for_job(): job_id = {job_id}, interval = {interval}, timeout = {timeout}"
        )
        ...
        while datetime.utcnow() < max_end_time:
            ...
            display.vvvv(f"poll_for_job(): job_id = {job_id}, status = {status.text}")
        ...
        raise TimedOutException(f"Timed out waiting for job id {job_id}")
    ...
    def send_request(
        ...
        **message_kwargs,
        ...
        display.vvvv(f"send_request(): headers = {headers}")
        display.vvvv(f"send_request(): method = {method}")
        display.vvvv(f"send_request(): path = {path}")
        display.vvvv(f"send_request(): data = {data}")
        ...
        try:
            ...
            display.vvvv(f"send_request(): response code = {response.getcode()}")
    ...
    def _validate_response(http_code, http_response):
        ...
        display.vvvv(f"_validate_response(): response = {data}")
