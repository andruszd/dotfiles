...
class ActionModule(ActionBase):
    ...
    def _get_latest_content_version(self, content_type):
        ...
        cmd = f"<request><{content_type}><upgrade><check/></upgrade></{content_type}></request>"
    ...
    def _download_install_content(self, content_type):
        ...
        install = (
            ...
            f"<version>latest</version><commit>no</commit>"
            f"</install></upgrade></{content_type}></request>"
        )
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        for content_type in content_types:
            ...
            display.debug(f"panos_dynamic_updates: checking {content_type}")

            latest_version = self._get_latest_content_version(content_type)

            if latest_version is not None:
                display.debug(
                    f"panos_dynamic_updates: install version {latest_version} of {content_type}"
                )
