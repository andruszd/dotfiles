...
options:
    ...
    edit:
        description:
            - If C(true), replace any existing configuration at the specified
              location with the contents of I(element).
            - If C(false), merge the contents of I(element) with any existing
...
def iterpath(node, tag=None, path="."):
    ...
    for child in node:
        if child.tag == "entry":
            _child_path = f"{path}/{child.tag}[@name='" f"{child.attrib['name']}" f"']"
        else:
            _child_path = f"{path}/{child.tag}"
